...
async function analyzeBatch(articles) {
  ...
  console.log('[claude] analyzeBatch called', {
...
The array should have exactly ${articles.length} objects in the same order as the articles above.`;
  ...
  try {
    console.log('[claude] Creating message with model:', 'claude-haiku-4-5-20251001');
    ...
    console.log('[claude] Message created successfully');
    ...
    try {
      ...
      sentiments = JSON.parse(jsonText);
...
export async function analyzeSentiment(articles) {
  ...
  try {
    ...
    const allSentiments = await Promise.all(
    ...
    batches.forEach((batch, batchIdx) => {
      const sentiments = allSentiments[batchIdx];
